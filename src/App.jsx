...
function App() {
  ...
  return (
    <div className={app.app} onClick={() => (cursorPos ? (shift < texts.length - 1 ? setShift((state) => state + 1) : "") : shift > 0 ? setShift((state) => state - 1) : "")}>
      <div className={[app.logo, app.text18].join(" ")}>
        <div>Michał</div>
        <div>Górnicki</div>
      </div>

      <img className={app.cursor} style={{ left: cursorCords[1] - 30, top: cursorCords[0] - 50, transition: "0.5s", rotate: cursorPos ? "0deg" : "180deg" }} src={arrow} alt="" />
      ...
      {texts.map((item, index) => (
        <div className={[app.container, index < shift ? app.shifted0 : index === shift ? app.shifted1 : index > shift ? app.shifted2 : ""].join(" ")}>{item}</div>
      ))}
    </div>
  );
}
