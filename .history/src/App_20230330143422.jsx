...
function App() {
  ...
  const texts = [
    ...
    [
      <>
        ...
        <div className={text.sectionContainer}>
          <div className={text.text}>My default text</div>
          <div className={[text.text, app.selected].join(" ")}>My default text</div>
          <div className={text.text}>My default text</div>
        </div>
      </>,
    ],
  ...
  return (
    <div className={app.app}>
      <div className={[app.logo, text.text18].join(" ")}>
        <div>Michał</div>
        <div>Górnicki</div>
      </div>
      ...
      {texts.map((item, index) => (
        <div className={[text.container, index < shift ? text.shifted0 : index === shift ? text.shifted1 : index > shift ? app.shifted2 : ""].join(" ")}
        onClick={() => (cursorPos ? (shift < texts.length - 1 ? setShift((state) => state + 1) : "") : shift > 0 ? setShift((state) => state - 1) : "")}
        >{item}</div>
      ))}
    </div>
